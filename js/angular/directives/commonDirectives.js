  ...
  return {
    ...
    link: function (scope) {
      ...
      scope.$watchGroup(['totalPages', 'currentPage'], function () {
        ...
        if (scope.totalPages > 0 && scope.currentPage > scope.totalPages) scope.currentPage = scope.totalPages;

        var start = Math.max(1, scope.currentPage - 2);
        var end = Math.min(scope.totalPages, scope.currentPage + 2);

        // Adjust if near start or end to show constant number of buttons if possible
        if (start === 1) {
          end = Math.min(scope.totalPages, 5);
        }
        if (end === scope.totalPages) {
          start = Math.max(1, scope.totalPages - 4);
        }
